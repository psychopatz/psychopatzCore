...
function PsychopatzProfilerWindow:refreshMetrics(force)
    ...
    if self.view == "Overview" then
        ...
        for _, metric in ipairs(profiler.GetMetrics("timer")) do
            ...
            local selfMsPerSec = metric.selfMsPerSec or metric.msPerSec
            cpu = cpu + selfMsPerSec
            namespaceCPU[metric.namespace] = (namespaceCPU[metric.namespace] or 0) + selfMsPerSec
        end
    ...
    elseif self.view == "CPU" then
        ...
        table.sort(timers, function(left, right)
            return (left.selfMsPerSec or left.msPerSec) > (right.selfMsPerSec or right.msPerSec)
        end)
        for _, metric in ipairs(timers) do
            add(self.metricList, metric.name, string.format("%.2f self / %.2f total ms/s | %.2f self avg | %.2f peak | %.1f/s",
                metric.selfMsPerSec or metric.msPerSec, metric.msPerSec,
                metric.averageSelfMs or metric.averageMs, metric.selfPeakMs or metric.peakMs,
                metric.callsPerSec))
        end
